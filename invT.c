...
double* MATRIX_INV(double* array, int n){
    ...
    #pragma omp parallel for private(i,j,k) shared(R,Q,array) num_threads(N_THREAD)
    for(i=0;i<n;i++){
        for(j=0;j<=i;j++){
    ...
    for(i=1;i<n;i++){
        ...
        for(j=0;j<i;j++){
            ...
            SIMDptr2 = &R[j*n];
            SIMDptr4 = &Rinv[j*n];
            #pragma omp target enter data map(to:SIMDptr1[0:n],SIMDptr2[0:n],coeff) //#pragma omp simd aligned(SIMDptr1,SIMDptr2:64)
            #pragma omp target
            for(k=0;k<n;k++){
                SIMDptr1[k]-=coeff*SIMDptr2[k];//#pragma omp critical
            }
    ...
    #pragma omp parallel for private(i,j,k) shared(Ainv,Q,Rinv) num_threads(N_THREAD)
    for(int k=0;k<n;k++){
        for(int i=0;i<n;i++){
            for(int j=0;j<n;j++){
